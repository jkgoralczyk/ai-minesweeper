...
class GeneticAlgorithm(object):
    ...
    def run(self):
        ...
        print("generation "+repr(i))
        for member in self.current_generation:
            print("chromosome:",member.genes,"fitness:", member.fitness)
    ...
    def last_generation(self):
        ...
        return ((member.genes) for member
...
data = [('bomba atomowa', 10),
        ('bomba biologiczna', 20),
        ('bomba jądrowa', 45),
        ('bomba termojądrowa', 89),
        ('mina przeciwpiechotna', 67),
        ('mina przeciwdenna', 40),
        ('granat ręczny', 28),
        ('granat odłamkowy', 45),
        ]
...
def fitness (individual, data):
    fitness = 0
    if individual.count(1) == bb:
        for (selected, (alpha, profit)) in zip(individual, data):
            if selected:
                fitness += profit
    return fitness
...
with open('data/genetics/bombs.csv', 'w') as csvfile:
    filewriter = csv.writer(csvfile, delimiter=',',
                            quotechar=',', quoting=csv.QUOTE_MINIMAL)
    filewriter.writerow(ga.best_individual())